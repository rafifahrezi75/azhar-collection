...
const ProductDetailModal = memo(function ProductDetailModal({
    ...
    const universalMaterials = materials.filter((m) => !m.size_name || m.size_name === "ALL");
    const distinctSizesWithMaterials = sizes.length > 0
        ? sizes.map((s) => ({
              size_name: s.size_name,
              price: s.price,
              materials: materials.filter((m) => m.size_name === s.size_name),
          }))
        : [];
    ...
    return (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50 animate-in fade-in duration-200 overflow-y-auto">
            <div className="bg-white rounded-xl max-w-3xl w-full p-4 sm:p-6 shadow-2xl space-y-5 border border-slate-200 animate-in zoom-in-95 duration-150 my-auto max-h-[92vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 space-y-1">
                        <span className="text-[11px] font-semibold text-slate-500 block">Harga Dasar Standar</span>
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                                <thead>
                                    <tr className="bg-slate-100/80 border-b border-slate-200 text-[11px] font-bold text-slate-700 uppercase tracking-wider">
                                        ...
                                        <th className="py-2 px-3 w-32">Ukuran</th>
                                        <th className="py-2 px-3 w-40">Harga Jual</th>
                                        <th className="py-2 px-3">Keterangan Spesifikasi</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {sizes.map((s, sIdx) => (
                                        <tr key={s.id || sIdx} className="hover:bg-slate-50/60">
                                            ...
                                            <td className="py-2 px-3 font-bold text-slate-900">{s.size_name}</td>
                                            <td className="py-2 px-3 font-bold text-teal-700 font-mono">{formatCurrency(s.price)}</td>
                                            <td className="py-2 px-3 text-slate-600">{s.notes || "-"}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="space-y-3">
                    ...
                    {materials.length === 0 ? (
                        <div className="p-6 text-center bg-slate-50 rounded-lg border border-dashed border-slate-200 text-xs text-slate-400">
                            ...
                            <p className="text-[11px] text-slate-400">Klik tombol Edit untuk menghubungkan bahan baku gudang.</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {/* Render Size-Based BOM Cards if sizes exist */}
                            {distinctSizesWithMaterials.length > 0 && distinctSizesWithMaterials.map((sz, szIdx) => (
                                <div key={szIdx} className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-2xs">
                                    <div className="bg-slate-50 px-3.5 py-2 border-b border-slate-200 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className="w-6 h-6 rounded bg-teal-600 text-white font-bold text-xs flex items-center justify-center">
                                                {sz.size_name}
                                            </span>
                                            <span className="text-xs font-bold text-slate-900">
                                                Resep Ukuran {sz.size_name}
                                            </span>
                                            {sz.price > 0 && (
                                        ...
                                        <span className="text-[11px] text-slate-500 font-semibold">
                                            {sz.materials.length} Bahan
                                        </span>
                                    </div>

                                    {sz.materials.length === 0 ? (
                                        <div className="p-3 text-xs text-slate-400 italic text-center">
                                            Belum ada resep bahan khusus untuk ukuran {sz.size_name}.
                                        </div>
                                    ) : (
                                        <div className="divide-y divide-slate-100">
                                            {sz.materials.map((mat, mIdx) => (
                                                <div key={mat.id || mIdx} className="px-3.5 py-2 flex items-center justify-between text-xs hover:bg-slate-50/50">
                                                    <div>
                                                        <span className="font-bold text-slate-900 block">{mat.item?.name || "Bahan Baku"}</span>
                                                        <span className="text-[10px] text-slate-500 font-mono">
                                                            {mat.item?.code || "-"} {mat.notes ? `(${mat.notes})` : ""}
                                                        </span>
                                                    </div>
                                                    <div className="text-right">
                                                        <span className="font-bold text-teal-700 font-mono block">
                                                            {mat.required_qty} {mat.unit_name || mat.item?.unit?.name || "Meter"}
                                                        </span>
                                                        <span className="text-[10px] text-slate-400">
                                                            Stok: {mat.item?.real_stock ?? 0} {mat.item?.unit?.name || ""}
                                                        </span>
                                                    </div>
                                                </div>
                                            ))}
                            ...
                            {/* Universal / Standard Materials Card */}
                            {universalMaterials.length > 0 && (
                                <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-2xs">
                                    ...
                                    <div className="divide-y divide-slate-100">
                                        {universalMaterials.map((mat, mIdx) => (
                                            <div key={mat.id || mIdx} className="px-3.5 py-2 flex items-center justify-between text-xs hover:bg-slate-50/50">
                                                <div>
                                                    <span className="font-bold text-slate-900 block">{mat.item?.name || "Bahan Baku"}</span>
                                                    <span className="text-[10px] text-slate-500 font-mono">
