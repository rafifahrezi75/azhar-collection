...
export default function Edit({ item, categories = [], units = [] }) {
    ...
    const [directRealQty, setDirectRealQty] = useState(item?.real_stock ?? "");
    const [directEstQty, setDirectEstQty] = useState(
        item?.estimated_stock ? String(item.estimated_stock) : "",
    );
    ...
    const [form, setForm] = useState({
        ...
        stock:
            item?.stock ??
            (item?.real_stock ?? 0) + (item?.estimated_stock ?? 0),
        real_stock: item?.real_stock ?? 0,
        estimated_stock: item?.estimated_stock ?? 0,
        is_estimated_stock:
            (item?.estimated_stock ?? 0) > 0 ||
    ...
    useEffect(() => {
        ...
        setDirectRealQty(realStockVal > 0 ? realStockVal : 0);
        setDirectEstQty(estStockVal > 0 ? String(estStockVal) : "");
        setDirectEstUnitId(String(item?.unit_id || ""));
        const initialQ = { base_real: realStockVal, base_est: estStockVal };
        if (mappedConversions.length > 0) {
            setStockInputMode("multi");
            mappedConversions.forEach((c, idx) => {
                initialQ[`conv_real_${idx}`] = Number(c.real_stock || 0);
                initialQ[`conv_est_${idx}`] = Number(c.estimated_stock || 0);
            });
        } else setStockInputMode("direct");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post(`/api/items/${item.id}`, formData, {
                headers: { "Content-Type": "multipart/form-data" },
            });
            Toast.success(
                res.data.message || "Data barang berhasil diperbarui.",
            );
    ...
    return (
        <DashboardLayout>
            <Head title={`Edit ${item.name} - Azhar Collection`} />
                ...
                >
                    <div className="p-4 sm:p-5">
                            ...
                            <div className="flex items-center gap-2.5 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-[11px] text-slate-500 truncate">
                                        Perbarui informasi bahan baku, kemasan multi-satuan, dan stok.
                                    </p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                                ...
                                <div className="p-4 bg-slate-50/50 rounded-lg border border-slate-200 space-y-3.5">
                                        ...
                                        <div className="border border-dashed border-slate-300 hover:border-teal-500 rounded-lg p-3 bg-white transition-colors flex items-center gap-3">
                                            ...
                                            <div className="flex-1 space-y-1">
                                                <input
                                                    ...
                                                    className="hidden"
                                                    id="barang_photo_edit"
                                                />
                                                <label
                                                    htmlFor="barang_photo_edit"
                                    ...
                                    <div className="flex items-center gap-2 pt-2 border-t border-slate-200/80">
                                        <input
                                            type="checkbox"
                                            name="is_active"
                                            id="edit_barang_is_active"
                                            checked={form.is_active}
                                            onChange={onChange}
                                            className="rounded border-slate-300 text-teal-600 focus:ring-teal-500 w-4 h-4 cursor-pointer"
                                        />
                                        <label
                                            htmlFor="edit_barang_is_active"
                            ...
                            {/* KOLOM KANAN: Multi-Satuan & Perhitungan Stok */}
                            <div className="lg:col-span-6 space-y-4">
                                            ...
                                            {form.conversions.map((conv, index) => (
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors shrink-0 cursor-pointer"
                                                        title="Hapus Kemasan"
                                                    >
                                ...
                                {/* Section Input / Adjustment Stok */}
                                <div className="p-4 bg-slate-50/50 rounded-lg border border-slate-200 space-y-3.5">
                                    <div className="flex items-center justify-between border-b border-slate-200/80 pb-2">
                                        <div className="flex items-center gap-2">
                                            <Calculator className="w-4 h-4 text-teal-600" />
                                            <label className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                                                Penyesuaian Stok ({baseUnitSymbol})
                                            </label>
                                        </div>
                                                ...
                                                <div className="flex rounded-lg shadow-2xs">
                                                    ...
                                                    {form.conversions && form.conversions.length > 0 ? (
                                                        <select
                                                            ...
                                                            className="border border-l-0 border-amber-300 rounded-r-lg px-2 py-1.5 bg-amber-50 text-amber-950 font-bold text-xs focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 shrink-0 cursor-pointer"
                                                        >
